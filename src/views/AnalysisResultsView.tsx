...
export function AnalysisResultsView({ nodes, onClose, onOpenProposal, onSelectCard, proposal }: AnalysisResultsViewProps) {
  const report = useMemo(() => buildAnalysisReport(nodes), [nodes])
  return <>
    ...
    <PanelScrollArea className="reports-panel-content" label="Analysis results content">
      ...
      <section className="analysis-results-overview">
        <div><strong>{report.risks.length}</strong><small>Risks</small></div>
        <div><strong>{report.inspectedAssets}/{report.totalAssets}</strong><small>Inspected</small></div>
        <div><strong>{report.aggregateProfiles}</strong><small>Profiled</small></div>
        <div><strong>{report.coverageGaps}</strong><small>Coverage gaps</small></div>
      </section>
      ...
      <div className="reports-heading"><strong>Material findings</strong><small>{report.risks.length} risk{report.risks.length === 1 ? '' : 's'}</small></div>
      {report.risks.length ? <div className="analysis-result-risks">{report.risks.map((risk) => <button aria-label={`Inspect result ${risk.title}`} className={`severity-${risk.severity}`} key={risk.nodeId} onClick={() => onSelectCard(risk.nodeId)} type="button">
        <ShieldAlert size={16} />
        <span><small>{risk.domain} · {risk.severity}</small><strong>{risk.title}</strong><p>{risk.detail}</p><dl>
          ...
          {risk.affectedAssets !== undefined && <div><dt>Assets</dt><dd>{risk.affectedAssets}</dd></div>}
      ...
      {report.evidence.length ? <div className="analysis-result-evidence">{report.evidence.map((item) => <button aria-label={`Inspect evidence ${item.title}`} key={item.nodeId} onClick={() => onSelectCard(item.nodeId)} type="button">
        ...
        <span><small>{item.kind}</small><strong>{item.title}</strong><p>{item.detail}</p></span>
