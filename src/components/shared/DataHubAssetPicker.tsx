...
interface DataHubAssetPickerProps {
  ...
  onInspect(assetRef: string, force?: boolean, connectorId?: string): Promise<{ asset: DataHubAssetSummary }>
  onOpenSettings(): void
...
export function DataHubAssetPicker({ connected, onBind, onInspect, onOpenSettings, onSearch }: DataHubAssetPickerProps) {
  ...
  const inspect = async (asset: DataHubAssetSummary, force = false) => {
    setBusy(true)
    setMessage(force ? 'Refreshing schema, governance and lineage…' : 'Loading trusted schema and classifications…')
    try {
      const detail = await onInspect(asset.assetRef ?? asset.urn, force, asset.connectorId)
  ...
  return <section className="datahub-picker">
    ...
    {results.length > 0 && <ol className="datahub-search-results">{results.map((asset) => <li key={`${asset.connectorId ?? 'datahub'}:${asset.assetRef ?? asset.urn}`}><button aria-current={preview?.connectorId === asset.connectorId && (preview?.assetRef ?? preview?.urn) === (asset.assetRef ?? asset.urn) ? 'true' : undefined} disabled={busy} onClick={() => void inspect(asset)} type="button"><span><strong>{asset.name}</strong><small>{asset.sourceSystem ?? 'Catalog'} · {asset.platform} · {asset.environment}</small></span><em>{asset.owners[0] ?? 'Owner unavailable'}</em><p>{asset.description}</p></button></li>)}</ol>}
    {preview && <article className="datahub-asset-preview">
      <header><div><small>PREVIEW BEFORE BINDING</small><h4>{preview.name}</h4><code>{preview.urn}</code></div><button aria-label="Refresh catalog metadata" disabled={busy} onClick={() => void inspect(preview, true)} type="button"><RefreshCw size={13} /></button></header>
