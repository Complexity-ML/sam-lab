...
export function buildPipelineAgentRequest(input: AgentContextInput & {
  ...
  return {
    ...
    domainPolicy: {
      product: 'SAM LAB is a Software Asset Management product, not a data-governance or privacy-analysis product.',
      select: 'Use only software inventory, application, vendor, license, seat, subscription, entitlement, contract, usage, spend, reclamation, compliance or renewal evidence.',
      exclude: 'Do not select unrelated business datasets because they contain PII or generic data-quality signals. Sensitive metadata is a handling guardrail only.',
      result: 'Report supported product, purchased/assigned/active seats, inactive or duplicate assignments, entitlement or renewal exposure, annual cost or savings, evidence gaps and owner action. Never invent values.',
    },
