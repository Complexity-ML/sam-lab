...
type ScenarioPresetId = 'pii-masking' | 'schema-drift' | 'broken-governance' | 'sam-evidence-gap' | 'license-reclamation' | 'compliance-exposure' | 'renewal-optimization'
...
export const scenarioPresets: Record<ScenarioPresetId, ScenarioPreset> = {
  ...
  'sam-evidence-gap': {
    title: 'SAM evidence gap',
    nodes: [
      { id: 'sam-gap-source', type: 'pipeline', position: { x: 100, y: 220 }, data: { kind: 'source', label: 'Unowned software subscriptions', description: 'Software subscription inventory with missing owner and incomplete entitlement evidence.', owner: 'Unassigned', status: 'blocked', schema: [{ name: 'product', type: 'string' }, { name: 'assigned_seats', type: 'number' }, { name: 'annual_cost', type: 'number' }, { name: 'contract_id', type: 'string' }], connectorId: 'catalog', sourceSystem: 'Catalog', assetRef: 'software/subscriptions/unowned', datahubTags: ['SAM', 'SUBSCRIPTION'], datahubQuality: 'unavailable', datahubFreshness: fresh } },
      { id: 'sam-gap-analysis', type: 'pipeline', position: { x: 430, y: 220 }, data: { kind: 'analysis', label: 'Verify entitlement evidence', description: 'Checks ownership, contract linkage, assignment totals and renewal coverage without inventing missing values.', owner: 'SAM LAB Agent', status: 'warning', schema: [], rule: 'require(owner + contract + assignments + renewal_date)' } },
      { id: 'sam-gap-review', type: 'pipeline', position: { x: 760, y: 220 }, data: { kind: 'review', label: 'Assign evidence owner', description: 'Procurement reviews the evidence gap before any compliance or optimization conclusion.', owner: 'Procurement', status: 'draft', schema: [], rule: 'require=owner_and_contract_confirmation' } },
      { id: 'sam-gap-output', type: 'pipeline', position: { x: 1090, y: 220 }, data: { kind: 'output', label: 'SAM evidence remediation register', description: 'Tracks missing ownership, entitlement evidence and accountable remediation actions.', owner: 'SAM Team', status: 'draft', schema: [] } },
    ],
    edges: [
      { id: 'sam-e-gap-analysis', source: 'sam-gap-source', target: 'sam-gap-analysis', type: 'elastic' },
      { id: 'sam-e-gap-review', source: 'sam-gap-analysis', target: 'sam-gap-review', type: 'elastic' },
      { id: 'sam-e-gap-output', source: 'sam-gap-review', target: 'sam-gap-output', type: 'elastic' },
    ],
  },
  'license-reclamation': {
    title: 'Copilot license optimization',
    nodes: [
      { id: 'sam-license-source', type: 'pipeline', position: { x: 80, y: 210 }, data: { kind: 'source', label: 'Copilot license utilization', description: 'DataHub-backed product snapshot: 300 purchased, 250 assigned and 178 active seats. The source contains aggregates, not GitHub logins or prompts.', owner: 'Developer Platform', status: 'healthy', schema: [{ name: 'product', type: 'string' }, { name: 'purchased_seats', type: 'number' }, { name: 'assigned_seats', type: 'number' }, { name: 'active_seats', type: 'number' }, { name: 'inactive_60d_plus', type: 'number' }, { name: 'never_active', type: 'number' }, { name: 'annual_unit_cost', type: 'number' }, { name: 'annualized_active_gap', type: 'number' }, { name: 'renewal_date', type: 'timestamp' }], datahubUrn: 'urn:li:dataset:(urn:li:dataPlatform:postgres,sam-copilot-demo.sam_copilot.sam_mart.license_utilization,PROD)', datahubPlatform: 'postgres', datahubEnvironment: 'PROD', datahubDomain: 'Software Asset Management', datahubTags: ['SAM', 'SYNTHETIC', 'PSEUDONYMIZED', 'LICENSE_USAGE'], datahubQuality: 'healthy', datahubFreshness: fresh, datahubUpstream: [{ urn: 'urn:li:dataset:(urn:li:dataPlatform:postgres,sam-copilot-demo.sam_copilot.sam_mart.license_assignment_snapshot,PROD)', name: 'license_assignment_snapshot', sensitive: true }], datahubDownstream: [{ urn: 'urn:li:dataset:(urn:li:dataPlatform:postgres,sam-copilot-demo.sam_copilot.sam_mart.reclaim_candidates,PROD)', name: 'reclaim_candidates', sensitive: true }, { urn: 'urn:li:dataset:(urn:li:dataPlatform:postgres,sam-copilot-demo.sam_copilot.sam_mart.renewal_risk,PROD)', name: 'renewal_risk', sensitive: false }], samAsset: { id: 'github-copilot-business', product: 'GitHub Copilot Business', vendor: 'GitHub', owner: 'Developer Platform', purchasedSeats: 300, assignedSeats: 250, activeSeats: 178, annualUnitCost: 228, renewalDate: '2026-09-15', complianceStatus: 'compliant', approved: true } } },
      { id: 'sam-normalize-assets', type: 'pipeline', position: { x: 390, y: 210 }, data: { kind: 'transform', label: 'Validate Copilot evidence', description: 'Checks the contract snapshot, pseudonymous seat keys, activity window and catalog lineage before analysis.', owner: 'SAM LAB Agent', status: 'healthy', schema: [], rule: 'require(contract + seat_assignments + usage_28d + owner + lineage)' } },
      { id: 'sam-usage-analysis', type: 'pipeline', position: { x: 700, y: 210 }, data: { kind: 'analysis', label: 'Find inactive Copilot seats', description: 'Identifies 42 seats inactive for at least 60 days or never used while keeping individual activity behind the review boundary.', owner: 'SAM LAB Agent', status: 'warning', schema: [], rule: 'candidate=never_active OR days_since_last_activity>=60' } },
      { id: 'sam-cost-impact', type: 'pipeline', position: { x: 1010, y: 210 }, data: { kind: 'impact', label: 'Calculate Copilot savings', description: 'Values 41 eligible seats at USD 9,348 annually and separates one business-critical investigation.', owner: 'SAM LAB Agent', status: 'warning', schema: [], rule: 'eligible_savings=sum(annual_unit_cost where candidate AND NOT critical_access)' } },
      { id: 'sam-reclaim-risk', type: 'pipeline', position: { x: 1320, y: 210 }, data: { kind: 'risk', label: 'Copilot reclamation risk', description: 'Prevents automatic removal and routes critical access, offboarding and ambiguous activity to the correct owner.', owner: 'SAM LAB Agent', status: 'warning', schema: [], rule: 'scope=github_copilot_business | risk_domain=governance | risk_type=data | severity=medium | confidence=0.92 | evidence=fresh | affected_assets=42 | action=owner_review_then_reclaim' } },
