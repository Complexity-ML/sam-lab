...
const REPOSITORY: &str = "Complexity-ML/sam-lab";
...
fn begin_install() -> Result<InstallGuard, String> {
    INSTALL_IN_PROGRESS
        ...
        .map(|_| InstallGuard)
        .map_err(|_| {
            "SAM LAB Setup is already installing. Keep this window open to follow its progress."
                .to_string()
        })
}
...
fn latest_app_release() -> Result<GitHubRelease, String> {
    let api_result = (|| {
        github_releases()?
            ...
            .ok_or_else(|| "SAM LAB has no published stable application release yet".to_string())
    })();
    api_result.or_else(|api_error| {
        ...
        let tag_name = response
            ...
            .to_string();
        let asset_names = [
            "SAM-LAB-Setup-arm64-helper",
            "SAM-LAB-Setup-arm64-helper.sha256",
            "SAM-LAB-Setup-x64-helper.exe",
            "SAM-LAB-Setup-x64-helper.exe.sha256",
            "SAM-LAB-Setup-x64.AppImage",
            "SAM-LAB-Setup-x64.AppImage.sha256",
        ];
        Ok(GitHubRelease {
            tarball_url: format!(
...
fn latest_setup_release() -> Result<GitHubRelease, String> {
    github_releases()?
        ...
        .ok_or_else(|| "SAM LAB has no published Setup release yet".to_string())
}
...
fn install_root() -> Result<PathBuf, String> {
    #[cfg(target_os = "windows")]
    {
        return dirs::data_local_dir()
            .map(|path| path.join("SAM LAB").join("setup-data"))
            .ok_or_else(|| "No local application-data directory is available".to_string());
    }
    #[cfg(not(target_os = "windows"))]
    dirs::data_local_dir()
        .map(|path| path.join("SAM LAB Setup"))
        .ok_or_else(|| "No local application-data directory is available".to_string())
}
...
fn electron_user_data() -> Result<PathBuf, String> {
    #[cfg(target_os = "macos")]
    {
        return dirs::home_dir()
            .map(|path| path.join("Library/Application Support/SAM LAB"))
            .ok_or_else(|| "No home directory is available".to_string());
    }
    #[cfg(target_os = "windows")]
    {
        return dirs::config_dir()
            .map(|path| path.join("SAM LAB"))
...
fn app_destination() -> Result<PathBuf, String> {
    ...
    {
        return dirs::home_dir()
            .map(|path| path.join("Applications/SAM LAB.app"))
            .ok_or_else(|| "No home directory is available".to_string());
    }
    #[cfg(target_os = "windows")]
    {
        return dirs::data_local_dir()
            .map(|path| path.join("Programs/SAM LAB"))
            .ok_or_else(|| "No local application-data directory is available".to_string());
    }
    #[cfg(target_os = "linux")]
    {
        return dirs::data_local_dir()
            .map(|path| path.join("SAM LAB").join("app"))
            .ok_or_else(|| "No XDG application-data directory is available".to_string());
    }
    #[allow(unreachable_code)]
    Err("SAM LAB Setup currently supports macOS, Windows and Linux".to_string())
}
...
fn helper_destination() -> Result<PathBuf, String> {
    ...
    Ok(electron_user_data()?
        .join("installer")
        .join(format!("sam-lab-setup{extension}")))
}
...
fn synchronize_electron_channel(channel: &str) -> Result<(), String> {
    ...
    fs::write(path, normalized_channel(Some(channel)))
        .map_err(|error| format!("Unable to synchronize the SAM LAB update channel: {error}"))
}

fn setup_helper_asset() -> Result<&'static str, String> {
    match (env::consts::OS, env::consts::ARCH) {
        ("macos", "aarch64") => Ok("SAM-LAB-Setup-arm64-helper"),
        ("macos", "x86_64") => Ok("SAM-LAB-Setup-x64-helper"),
        ("windows", "x86_64") => Ok("SAM-LAB-Setup-x64-helper.exe"),
        ("windows", "aarch64") => Ok("SAM-LAB-Setup-arm64-helper.exe"),
        ("linux", "x86_64") => Ok("SAM-LAB-Setup-x64.AppImage"),
        _ => Err(format!(
...
fn relaunch_latest_setup(
    ...
    emit(
        ...
        "Setup update",
        format!(
            "Updating SAM LAB Setup to {} before continuing…",
            release.tag_name
        ),
        8,
    );
...
fn built_application(source: &Path) -> PathBuf {
    ...
    source.join("release").join(folder).join("SAM LAB.app")
}
...
fn copy_application(source: &Path, destination: &Path) -> Result<(), String> {
    ...
    let status = background_command("robocopy")
        ...
        .arg("/NDL")
        .status();
    let status = status.map_err(|error| format!("Unable to copy SAM LAB: {error}"))?;
    #[cfg(any(target_os = "macos", target_os = "linux"))]
...
fn launch_application(destination: &Path) -> Result<(), String> {
    ...
    background_command("/usr/bin/open")
        ...
        .map_err(|error| error.to_string())?;
    #[cfg(target_os = "windows")]
    background_command(destination.join("SAM LAB.exe"))
        .spawn()
        .map_err(|error| error.to_string())?;
    #[cfg(target_os = "linux")]
...
fn install_linux_integration(destination: &Path, source: &Path) -> Result<(), String> {
    ...
    let icon = destination.join("resources").join("sam-lab.png");
    ...
    let desktop_entry = format!(
        "[Desktop Entry]\nType=Application\nVersion=1.0\nName=SAM LAB\nComment=Atomic neural architecture laboratory\nExec=\"{}\"\nIcon={}\nTerminal=false\nCategories=Development;Science;\nStartupWMClass=SAM LAB\n",
        executable.display().to_string().replace('"', "\\\""),
        icon.display()
    );
    fs::write(applications.join("sam-lab.desktop"), desktop_entry)
...
fn install_windows_integration(destination: &Path, release_tag: &str) -> Result<(), String> {
    let executable = destination.join("SAM LAB.exe");
    ...
    let uninstall_script = install_root()?.join("uninstall-sam-lab.ps1");
    let uninstall_body = format!(
        "$ErrorActionPreference = 'SilentlyContinue'\n\
         Stop-Process -Name 'SAM LAB' -Force\n\
         Start-Sleep -Milliseconds 400\n\
         Remove-Item -LiteralPath '{}' -Recurse -Force\n\
         $shell = New-Object -ComObject WScript.Shell\n\
         Remove-Item -LiteralPath ([IO.Path]::Combine([Environment]::GetFolderPath('Programs'), 'SAM LAB.lnk')) -Force\n\
         Remove-Item -LiteralPath ([IO.Path]::Combine([Environment]::GetFolderPath('Desktop'), 'SAM LAB.lnk')) -Force\n\
         Remove-Item -LiteralPath 'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\SAM LAB' -Recurse -Force\n",
        powershell_literal(destination)
    );
    fs::write(&uninstall_script, uninstall_body)
        .map_err(|error| format!("Unable to create the SAM LAB uninstaller: {error}"))?;
    ...
    let script = format!(
        "$ErrorActionPreference = 'Stop'; \
         ...
         $shell = New-Object -ComObject WScript.Shell; \
         foreach ($link in @([IO.Path]::Combine([Environment]::GetFolderPath('Programs'), 'SAM LAB.lnk'), [IO.Path]::Combine([Environment]::GetFolderPath('Desktop'), 'SAM LAB.lnk'))) {{ \
           $shortcut = $shell.CreateShortcut($link); \
           $shortcut.TargetPath = $exe; \
           $shortcut.WorkingDirectory = '{destination_literal}'; \
           $shortcut.IconLocation = \"$exe,0\"; \
           $shortcut.Description = 'SAM LAB neural architecture laboratory'; \
           $shortcut.Save(); \
         }}; \
         $key = 'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\SAM LAB'; \
         New-Item -Path $key -Force | Out-Null; \
         New-ItemProperty -Path $key -Name DisplayName -Value 'SAM LAB' -PropertyType String -Force | Out-Null; \
         ...
         New-ItemProperty -Path $key -Name URLInfoAbout -Value 'https://www.complexity-ai.fr/sam-lab' -PropertyType String -Force | Out-Null; \
...
fn schedule_windows_setup_cleanup() -> Result<(), String> {
    ...
    let script = format!(
        "$ErrorActionPreference = 'SilentlyContinue'; \
         ...
         Remove-Item -LiteralPath ([IO.Path]::Combine([Environment]::GetFolderPath('Programs'), 'SAM LAB Setup.lnk')) -Force; \
         Remove-Item -LiteralPath ([IO.Path]::Combine([Environment]::GetFolderPath('Desktop'), 'SAM LAB Setup.lnk')) -Force; \
         $setupDirectories = @(); \
         Get-ItemProperty 'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\*' | \
           Where-Object {{ $_.DisplayName -eq 'SAM LAB Setup' }} | \
           ForEach-Object {{ \
             if ($_.InstallLocation -and ([IO.Path]::GetFileName($_.InstallLocation.TrimEnd('\\')) -eq 'SAM LAB Setup')) {{ $setupDirectories += $_.InstallLocation }}; \
...
fn prepare_application_handoff(app: &AppHandle) -> Result<(), String> {
    if let Some(window) = app.get_webview_window("main") {
        window
            .hide()
            .map_err(|error| format!("Unable to hide SAM LAB Setup: {error}"))?;
    }
    app.set_activation_policy(tauri::ActivationPolicy::Accessory)
        .map_err(|error| format!("Unable to remove SAM LAB Setup from the Dock: {error}"))?;
...
fn stop_running_application() -> Result<(), String> {
    ...
    let _ = background_command("taskkill")
        .args(["/IM", "SAM LAB.exe", "/T", "/F"])
        .output();
    for _ in 0..50 {
        let running = background_command("tasklist")
            .args(["/FI", "IMAGENAME eq SAM LAB.exe", "/NH"])
            .output()
            .map(|output| {
    ...
    Err("SAM LAB is still running. Close the application, then retry the update.".to_string())
}
...
fn activate_application(next: &Path, destination: &Path, previous: &Path) -> Result<(), String> {
    ...
    if had_previous {
        rename_application_directory(destination, previous)
            .map_err(|error| format!("Unable to preserve the previous SAM LAB build: {error}"))?;
    }
    if let Err(error) = rename_application_directory(next, destination) {
        ...
        return Err(format!(
            "Unable to activate the new SAM LAB build; the previous build was restored: {error}"
        ));
    }
    Ok(())
}
...
fn perform_install(app: &AppHandle, channel: &str) -> Result<InstallResult, String> {
    ...
    emit(
        ...
        "Release",
        if channel == "main" {
            "Checking the latest SAM LAB main commit…"
        } else {
            "Checking the latest SAM LAB release…"
        },
        5,
    );
    ...
    emit(
        app,
        "Build",
        "Building the native SAM LAB Electron application…",
        52,
    );
    #[cfg(target_os = "macos")]
    run_npm(
    ...
    emit(
        app,
        "Complete",
        "SAM LAB is ready. Launching the application…",
        100,
    );
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .expect("error while running SAM LAB Setup");
}
...
mod tests {
    ...
    fn activates_a_build_and_keeps_one_rollback_copy() {
        ...
        let destination = root.path().join("SAM LAB");
        let next = root.path().join("SAM LAB.next");
        let previous = root.path().join("SAM LAB.previous");
